...
export const MealCard = memo(
  function MealCard({
    ...
    const ingredients = useMemo(
      () => mealApiService.getMealIngredients(meal),
      [meal]
    );
    const tags = useMemo(
      () =>
            ...
            <div className="space-y-1">
              {ingredients.slice(0, 4).map((ingredient, index) => {
                const isMatched = matchedIngredients.some(
                  matched =>
                    matched.ingredient.toLowerCase() ===
  ...
  (prevProps, nextProps) => {
    ...
    return (
      ...
      JSON.stringify(prevProps.availableIngredients) ===
        JSON.stringify(nextProps.availableIngredients)
    );
  }
);
