...
export async function getRandomMeals(
  ...
  for (let i = 0; i < pageSize; i += batchSize) {
    const batch = Array.from({ length: Math.min(batchSize, pageSize - i) }, () => getRandomMeal(service));
...
export function getIngredientThumbnailUrl(_service: MealApiService, ingredient: string, size: 'small' | 'medium' | 'large' = 'medium'): string {
...
export function getIngredientDetails(service: MealApiService, ingredient: string): { name: string; thumbnail: string; description?: string } {
  return {
    name: ingredient,
